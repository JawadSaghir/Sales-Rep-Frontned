...
def load_persona(path) -> m.Persona:
    ...
    return m.Persona(
        ...
        age=str(d.get("age", "")),
        personality=_tuple(d.get("personality")),
...
def load_scenario(path) -> m.Scenario:
    ...
    return m.Scenario(
        meta=load_meta(d),
        call_type=str(_req(d, "call_type", path)),
...
def load_objection_card(path) -> m.ObjectionCard:
    ...
    return m.ObjectionCard(
        meta=load_meta(d),
        trigger=str(_req(d, "trigger", path)),
        emotion=str(d.get("emotion", "")),
        buyer_language=_tuple(_req(d, "buyer_language", path)),
