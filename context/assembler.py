...
def merge_objection_ids(
    default: tuple[str, ...], add: tuple[str, ...], remove: tuple[str, ...]
) -> tuple[str, ...]:
    ordered, seen = [], set()
...
def assemble(
    ...
    difficulty = loaders.load_difficulty(
        _path(data_dir, "difficulty", selection.difficulty)
    )
    call_type = loaders.load_call_type(
        _path(data_dir, "call_types", selection.call_type)
    )
    scorecard = loaders.load_scorecard(
        _path(data_dir, "scorecards", selection.scorecard)
    )

    ids = merge_objection_ids(
        scenario.default_objection_ids,
        selection.add_objection_ids,
        selection.remove_objection_ids,
    )
    cards = tuple(
        loaders.load_objection_card(_path(data_dir, "objections", i)) for i in ids
    )
    ...
    manifest = m.ContextManifest(
        version=1,
        renderer="buyer_v1",
        included_layers=included,
        omitted_layers=_OMITTED,
    )
    return ctx, manifest
