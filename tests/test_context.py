...
def test_real_content_loads():
    ...
    for oid in ["trust", "timing", "finances"]:
        assert loaders.load_objection_card(
            d / "objections" / f"{oid}.yaml"
        ).buyer_language
...
def test_merge_objection_ids_hybrid():
    out = assembler.merge_objection_ids(
        ("authority", "pricing"), ("security",), ("pricing",)
    )
    assert out == (
        "authority",
        "security",
    )  # add appended, remove dropped, order kept, deduped
...
def test_selection_from_metadata_parses_and_falls_back():
    ...
    md = (
        '{"persona_id":"charlie-ritenour","scenario_id":"charlie-ritenour",'
    ...
    assert (
        selection_from_metadata(None, fallback=DEFAULT_SELECTION) is DEFAULT_SELECTION
    )
    assert (
        selection_from_metadata("not json", fallback=DEFAULT_SELECTION)
        is DEFAULT_SELECTION
    )
