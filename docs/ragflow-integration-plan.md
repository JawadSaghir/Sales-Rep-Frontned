...
> Status: PLAN (not yet implemented). Companion to `knowledge_base.md`, `what_get_embed.md`, and `voice_ai_design.md`.
...
> **Confirmed choices (2026-07-15):** (1) Persona = **prospect** first. (2) RAGFlow host = **local dev only for now** (console-tested; cloud hosting deferred). (3) First cut = **cards-only** — offline-built character cards as the prompt; live retrieval tool deferred to a later phase. (4) Embeddings = **HuggingFace model via RAGFlow's built-in TEI** (e.g. `bge-m3` / `Qwen/Qwen3-Embedding-0.6B`).
...
| Persona (`prompts/agent-instructions.md`) | Plays the **rep** ("Alex"). Mission/docs want it to play the **objecting prospect**. References tools + a `rep_knowledge_base` that don't exist. |
...
| RAGFlow engine (`rag/`) | Full OSS clone, own git repo + Docker stack. **Unwired** — no import, no client, no submodule. |
