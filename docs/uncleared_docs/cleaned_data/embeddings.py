...
def cluster_vectors(vectors: list[list[float]], min_cluster_size: int = 5) -> list[int]:
    ...
    if len(x) >= 4 * min_cluster_size and x.shape[1] > 5:
        ...
        # init="random" skips the spectral eigensolver, which fails ("too small
        # an eigengap") on our small, near-disconnected neighbor graphs and
        # falls back to random anyway. Choosing it up front avoids the warning;
        # random_state keeps the layout reproducible.
        x = umap.UMAP(
            n_components=n_comp,
            init="random",
            random_state=42,
